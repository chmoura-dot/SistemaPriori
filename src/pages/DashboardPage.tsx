        ...
        <div className="bg-white border border-zinc-100 rounded-2xl p-5 shadow-sm">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            ...
            {inactivePatients.length > 0 && (
              <button onClick={() => { localStorage.setItem('customers_filter', 'sem-consulta-60d'); onNavigate('/clientes'); }} className="flex items-center gap-3 p-3 bg-zinc-50 border border-zinc-200 rounded-xl hover:border-zinc-300 transition-all text-left">
                <UserMinus size={18} className="text-zinc-500 shrink-0" />
                <div>
                  <p className="text-xs font-bold text-zinc-700">{inactivePatients.length} paciente{inactivePatients.length > 1 ? 's' : ''} sem consulta há +60 dias</p>
